...
namespace SimpleInventorySort
{
	...
	{
		private static readonly Dictionary<long, List<long>> m_conveyorCache = new Dictionary<long, List<long>>(10000);
		private static readonly Dictionary<long, long[]> m_conveyorConnected = new Dictionary<long, long[]>();
			...
			try {
				...
				foreach (IMyEntity entity in entities) {
					...
					foreach (MyObjectBuilder_ConveyorLine line in gridObject.ConveyorLines) {
						IMySlimBlock slimBlockStart = grid.GetCubeBlock(line.StartPosition);

						if (slimBlockStart == null || slimBlockStart.FatBlock == null || !slimBlockStart.FatBlock.IsFunctional) {
							continue;
						}

						IMySlimBlock slimBlockEnd = grid.GetCubeBlock(line.EndPosition);
		...
		private static void ConnectConveyorBlocks(IMySlimBlock slimBlockStart, IMySlimBlock slimBlockEnd)
		{
			...
			if (startList != null) {
				...
				List<long> newList = new List<long> {
					slimBlockStart.FatBlock.EntityId
				};
