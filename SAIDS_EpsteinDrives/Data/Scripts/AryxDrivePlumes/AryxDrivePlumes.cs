...
namespace Aryx.DrivePlumes
{
	...
	public class AryxDrivePlume : MyGameLogicComponent
	{
		...
		{
			if (!initialised && thrust.CubeGrid.Physics != null) //Don't simulate for non-grid shit.
			{
		...
		public override void UpdateAfterSimulation()
		{
			if (isValidEDrive)
			{
				...
				{
					float thrustPct = thrust.CurrentThrust / thrust.MaxThrust;

                    if (thrustPct >= 0.1f) thrusterFiring = true;
					...
					if (thrusterFiring && thrust.Enabled)
					{
						...
						else //If it does exist, keep it playing.
						{
							...
							particle.UserScale = particleSize * thrustPct;
							particle.Play();
						}
					...
					else
					{
						...
						if (emitter != null)
						{
							if (emitter.IsPlaying) emitter.StopSound(false, false);
						}
					}
				}
			}
